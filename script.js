...
/* симптомы: заголовок причины, описание, время, подсвечиваемые узлы */
var SYM_RU = [
  {c:"Утечка фреона или компрессор", d:"Чаще всего уходит фреон через микротрещину в контуре, реже - устал компрессор или забит капилляр. Найдём течеискателем и починим на месте.", t:"30-60 мин", p:["pt-evap","pt-comp","pt-cap"], l:"испаритель · компрессор"},
  {c:"Реле, плата или предохранитель", d:"Если холодильник молчит и лампочка не горит - смотрим пуско-защитное реле, плавкий предохранитель и плату управления. Детали с собой.", t:"10-30 мин", p:["pt-relay","pt-board"], l:"реле · плата"},
  {c:"Засор дренажа", d:"Вода под ящиками или на полу - забит слив талой воды. Прочищаем дренаж и проверяем испаритель, чтобы не повторилось.", t:"5-15 мин", p:["pt-drain"], l:"дренаж"},
  {c:"Вентилятор или компрессор", d:"Гул, дребезг и скрежет дают вентилятор испарителя, изношенный компрессор или ослабшие крепления. Определим по звуку и заменим деталь.", t:"10-20 мин", p:["pt-fan","pt-comp"], l:"вентилятор · компрессор"},
  {c:"Оттайка No Frost или уплотнитель", d:"Снежная шуба - не работает ТЭН, датчик или таймер оттайки, либо дверь не прилегает и тянет тёплый воздух. Меняем узел, регулируем дверь.", t:"10-30 мин", p:["pt-evap","pt-seal"], l:"испаритель · уплотнитель"},
  {c:"Пуско-защитное реле", d:"Щелчок и тишина каждые несколько минут - реле пытается запустить компрессор и уходит в защиту. Обычно хватает замены реле, реже нужен компрессор.", t:"5-10 мин", p:["pt-relay","pt-comp"], l:"реле · компрессор"},
  {c:"Плата или датчик температуры", d:"Код ошибки на дисплее - сбой датчика, платы управления или вентилятора. Считываем код, проверяем цепь и меняем неисправный узел.", t:"10-30 мин", p:["pt-board","pt-therm"], l:"плата · датчик"},
  {c:"Терморегулятор или утечка", d:"Компрессор не выключается, если терморегулятор не видит холода, уплотнитель пропускает тепло или ушёл фреон. Проверяем все три причины за один визит.", t:"10-20 мин", p:["pt-therm","pt-seal","pt-cond"], l:"терморегулятор · конденсатор"}
];
...
if (location.hash) {
  ...
  if (hel && hel.classList.contains("chip")) setTimeout(function(){ goTo(hid, false); }, 60);
}
})();
