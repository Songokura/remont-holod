...
var SYM_RU = [
  {c:"Утечка фреона, компрессор или нарушение циркуляции воздуха", d:"Чаще всего уходит фреон через микротрещину в контуре, реже - потеря производительности компрессора, забит капилляр или нарушение циркуляции воздуха. Найдём течеискателем и починим на месте.", t:"30-60 мин", p:["pt-evap","pt-comp","pt-cap","pt-fan"], l:"испаритель · компрессор · вентилятор", w:[]},
  {c:"Холодильник не включается", k:"", d:"", dp:["Если холодильник не работает и внутри не горит свет, в первую очередь проверьте розетку.","Подключите к ней любой другой бытовой прибор - например, зарядное устройство, фен или настольную лампу.","Если розетка исправна, вероятнее всего, неисправность находится в самом холодильнике. В этом случае потребуется диагностика мастера.","<b>📞 Вызовите меня - я проведу диагностику и определю причину неисправности.</b>"], t:"10-30 мин", p:["pt-relay","pt-board"], l:"реле · плата", w:[]},
  {c:"Засор дренажа", d:"Вода под ящиками или на полу - забит слив талой воды. Прочищаем дренаж и проверяем испаритель, чтобы не повторилось.", t:"5-15 мин", p:["pt-drain"], l:"дренаж", w:[]},
  {c:"Вентилятор или компрессор", d:"Гул, дребезг и скрежет дают вентилятор испарителя, изношенный компрессор или ослабшие крепления. Определим по звуку и заменим деталь.", t:"10-20 мин", p:["pt-fan","pt-comp"], l:"вентилятор · компрессор", w:[]},
  {c:"Оттайка No Frost или уплотнитель", d:"Снежная шуба - не работает ТЭН, датчик или таймер оттайки, либо дверь не прилегает и тянет тёплый воздух. Меняем узел, регулируем дверь.", t:"10-30 мин", p:["pt-evap","pt-seal"], l:"испаритель · уплотнитель", w:[]},
  {c:"Пуско-защитное реле", d:"", dp:["Компрессор пытается запуститься, но не выходит на рабочий режим - срабатывает тепловая защита, после чего попытка запуска повторяется через 5-10 минут.","Чаще всего причина - неисправное пусковое реле. Реже проблема связана с самим компрессором.","Точная причина определяется при диагностике."], t:"5-10 мин", p:["pt-relay","pt-comp"], l:"реле · компрессор", w:[]},
  {c:"Плата или датчик температуры", d:"Код ошибки на дисплее - сбой датчика, платы управления или вентилятора. Считываем код, проверяем цепь и меняем неисправный узел.", t:"10-30 мин", p:["pt-board","pt-therm"], l:"плата · датчик", w:[]},
...
function showSym(i){
  ...
  /* нет цен у симптома - прячем прайс и подпись, текст занимает место */
  var noPr = !(s.w && s.w.length), pn = document.querySelector(".sa-parts");
  if (pr) pr.hidden = noPr;
  if (pn) pn.hidden = noPr;
  if (d) d.classList.toggle("sa-d-lg", !!s.dp);
